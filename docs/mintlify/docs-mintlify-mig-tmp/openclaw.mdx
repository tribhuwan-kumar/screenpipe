...
<Note>
`screenpipe-mcp-http` is a binary inside the `screenpipe-mcp` npm package — it's not a separate package. The `-y` flag auto-installs it.
</Note>
...
When connected via MCP stdio, OpenClaw gets access to these tools. The HTTP server currently only exposes **search-content** — we're working on full parity.
